...
export const EventReplayView: React.FC = () => {
  const [selectedPreset, setSelectedPreset] = useState<HistoricalEventPreset>(PRESETS[0]);
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Run HydroGuard transparent risk scoring on historical Open-Meteo ERA5 atmospheric archives
            </p>
          </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                >
                  {preset.type === 'rainfall' ? 'Rainfall-Driven Flood' : 'Cryospheric Hazard (Out-of-Scope)'}
                </span>
              ...
              <p className={`mt-1 text-xs line-clamp-2 ${isSelected ? 'text-slate-300' : 'text-slate-600'}`}>
                {preset.summary}
              </p>
            </button>
          );
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-3">
          <div>
            ...
            <span className="text-[11px] text-slate-500">
              Interactive timeline with disaster moment marker. Click or hover any bar to inspect.
            </span>
          </div>

          {activePoint && (
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono text-slate-500">Inspecting:</span>
              <span className="font-mono font-bold text-xs text-slate-900">{activePoint.displayTime}</span>
              <span className={`px-2 py-0.5 rounded text-[11px] font-mono font-bold border ${getScoreColor(activePoint.riskScore)}`}>
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-2 text-slate-500">
            ...
            <span className="text-xs font-mono">Fetching ERA5 reanalysis archive from Open-Meteo...</span>
          </div>
        ) : errorMsg ? (
          ...
          <div className="space-y-4">
            ...
            <div className="relative pt-6 pb-2">
              ...
              <div className="absolute inset-x-0 top-6 bottom-8 pointer-events-none flex flex-col justify-between text-[9px] font-mono text-slate-400">
                <div className="border-b border-dashed border-red-200 flex justify-between pr-1">
                  <span className="text-red-600 font-bold bg-white/80 px-1">Severe (80)</span>
                </div>
                <div className="border-b border-dashed border-orange-200 flex justify-between pr-1">
                  <span className="text-orange-600 font-bold bg-white/80 px-1">High (60)</span>
                </div>
                <div className="border-b border-dashed border-amber-200 flex justify-between pr-1">
                  <span className="text-amber-600 font-bold bg-white/80 px-1">Medium (30)</span>
                </div>
                <div className="border-b border-slate-200 flex justify-between pr-1">
                  <span className="text-slate-400 bg-white/80 px-1">Baseline (0)</span>
                </div>
              </div>
              ...
              <div className="flex justify-between text-[10px] font-mono text-slate-500 mt-2 px-2 border-t border-slate-200 pt-1">
                ...
                <span>{timelineData[timelineData.length - 1]?.displayTime}</span>
              </div>
            </div>
            ...
            {activePoint && (
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-5 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs">
                <div>
                  <span className="text-[10px] text-slate-500 uppercase font-bold block">1h Rainfall</span>
                  <span className="font-mono font-bold text-slate-900 text-base">{activePoint.rain1h} mm</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 uppercase font-bold block">24h Cumulative</span>
                  <span className="font-mono font-bold text-slate-900 text-base">{activePoint.rain24h} mm</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 uppercase font-bold block">72h Antecedent</span>
                  <span className="font-mono font-bold text-slate-900 text-base">{activePoint.rain72h} mm</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 uppercase font-bold block">Soil Saturation</span>
                  <span className="font-mono font-bold text-emerald-700 text-base">{activePoint.soilPct}%</span>
                </div>
                <div className="col-span-2 sm:col-span-1 border-t sm:border-t-0 sm:border-l border-slate-200 pt-2 sm:pt-0 sm:pl-3">
                  <span className="text-[10px] text-slate-500 uppercase font-bold block">Model Tier</span>
                  <span className={`inline-block font-mono font-bold px-2 py-0.5 rounded text-xs border ${getScoreColor(activePoint.riskScore)}`}>
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50/80 p-4 text-xs text-amber-900 space-y-1.5 shadow-2xs">
        ...
        <p className="leading-relaxed">
          <strong>Limitation Note:</strong> Atmospheric reanalysis models (such as ERA5-Land and Open-Meteo Archive) operate at an effective grid resolution of approximately ~25 km. Consequently, hyper-localized orographic cloudbursts, narrow convective micro-cells, and isolated valley thermal updrafts may be smoothed out or significantly underestimated in reanalysis datasets compared to ground-truth automated weather stations (AWS).
        </p>
      </div>
    </div>
  );

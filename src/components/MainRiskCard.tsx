...
export const MainRiskCard: React.FC<MainRiskCardProps> = ({
  ...
  const flaggedVillages = villages.filter(
  ...
  const sectorNames =
    flaggedVillages.length > 0
      ? flaggedVillages.map((v) => v.name).join(", ")
      : "All villages below watch level";
  const exposedPopulation = villages.length;
      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 min-w-0 flex-wrap">
              <span
                className={`text-[10px] font-mono font-bold tracking-wider px-2 py-0.5 rounded shrink-0 ${theme.badgeBg}`}
              >
                {theme.code}
              </span>
        ...
        <div className="relative z-10 mt-3 sm:mt-0 flex items-center gap-2 sm:gap-3 shrink-0 flex-wrap">
          ...
          <div
            onClick={() =>
              onInspectMetric?.({
                title: "Data retrieval age",
                value: leadTime,
                unit: "range",
                status: "Provider retrieval timestamp",
                statusLevel: overallRisk,
                source: "Open-Meteo response timestamp",
                timestamp: lastUpdated,
                methodNote:
                  "Time since the latest successful provider retrieval. Model valid time is shown separately above the dashboard.",
                threshold:
                  "Refresh every 10 minutes; cached responses expire after 6 hours",
              })
            }
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 pt-1">
              <div
                onClick={() =>
                  onInspectMetric?.({
                    ...
                    timestamp: lastUpdated,
                    methodNote:
                      "Counts villages whose own score is in the High (60-79) or Severe (80-100) tier.",
              ...
              <div
                onClick={() =>
                  onInspectMetric?.({
                    ...
                    timestamp: lastUpdated,
                    methodNote:
                      "Elapsed time since the response was retrieved. This is not flood arrival or warning lead time.",
                    threshold: "Cache expiry: 6 hours",
                  })
                }
                className="rounded-xl bg-slate-50/90 p-3 border border-slate-200/80 hover:border-slate-300 transition-colors cursor-pointer"
              >
                ...
                <span className="text-[11px] text-slate-500 mt-0.5 block truncate">
                  Since last successful retrieval
                </span>
              </div>
              ...
              <div
                onClick={() =>
                  onInspectMetric?.({
                    title: "Monitored village locations",
                    value: exposedPopulation.toLocaleString("en-IN"),
                    unit: "villages",
                    status: "Configured coordinates",
                    statusLevel: overallRisk,
                    source: "HydroGuard catchment coordinate configuration",
                    timestamp: "Config",
                    methodNote:
                      "Number of configured village points covered by the current valid API response. Nearby points can share a model grid cell.",
                    threshold: "n/a",
                  })
                }
                className="rounded-xl bg-slate-50/90 p-3 border border-slate-200/80 hover:border-slate-300 transition-colors cursor-pointer"
              >
                ...
                <span className="font-bold text-slate-900 text-sm mt-1 block font-mono tabular-nums">
                  {exposedPopulation.toLocaleString("en-IN")} villages
                </span>
                <span className="text-[11px] text-slate-500 mt-0.5 block truncate">
                  Scored from current model inputs
                </span>
              </div>
              ...
              <div
                onClick={() =>
                  onInspectMetric?.({
                    ...
                    statusLevel: overallRisk,
                    source:
                      "HydroGuard tier-to-action mapping (prototype, not an official SOP)",
                    timestamp: lastUpdated,
                    methodNote:
                      "Suggested action for the current risk tier. It has not been reviewed by any disaster management authority.",
                    threshold:
                      "Tier: Low (0-29) | Medium (30-59) | High (60-79) | Severe (80-100)",
                  })
                }
                className="rounded-xl bg-slate-50/90 p-3 border border-slate-200/80 hover:border-slate-300 transition-colors cursor-pointer"
              >
          ...
          <div className="lg:col-span-4 rounded-xl bg-slate-50/90 p-4 border border-slate-200/90 flex flex-col justify-between space-y-3.5 shadow-2xs">
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                ...
                <span className="text-[9px] text-slate-400 block mt-0.5 font-mono">
                  {transparentScore?.signalAgreement.statusText ||
                    "Indicator agreement"}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] font-bold font-mono text-slate-500">
                <span
                  className={
                    riskScore < 30 ? "text-emerald-700 font-black" : ""
                  }
                >
                  LOW 0-29
                </span>
                ...
                <span
                  className={riskScore >= 80 ? "text-red-700 font-black" : ""}
                >
                  CRIT 80+
                </span>
              </div>
              <div className="grid grid-cols-4 gap-1 h-2.5">
                <div
                  className={`rounded-l-full transition-colors duration-500 ${riskScore >= 0 ? "bg-emerald-500" : "bg-slate-200"}`}
                />
                <div
                  className={`transition-colors duration-500 ${riskScore >= 30 ? "bg-amber-500" : "bg-slate-200"}`}
                />
                <div
                  className={`transition-colors duration-500 ${riskScore >= 60 ? "bg-orange-500" : "bg-slate-200"}`}
                />
                <div
                  className={`rounded-r-full transition-colors duration-500 ${riskScore >= 80 ? "bg-red-600" : "bg-slate-200"}`}
                />
              </div>
            ...
            >
              <span>View Emergency View village scores</span>
              <ArrowRight className="h-3.5 w-3.5" />
            </button>

...
export const MultiSourceCards: React.FC<MultiSourceCardsProps> = ({
  ...
  const displayCards = cards.map((c) =>
    isLive
      ? {
          ...
          plainText: c.metric.unavailable
            ? `No data: ${c.metric.unavailableReason ?? "unavailable"}.`
            : c.plainText,
        }
      : {
          ...c,
  ...
  const handleCardClick = (item: (typeof displayCards)[0]) => {
    onInspectMetric?.({
      ...
      source:
        item.metric.sourceName ||
        (item.metric.iconType === "rain"
          ? "Open-Meteo Weather Forecast API"
          : item.metric.iconType === "river"
            ? "Copernicus GloFAS / River Inundation API"
            : "Open-Meteo Land Surface Hydro Model"),
      timestamp:
        item.metric.sourceTimestamp ||
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Rainfall, river discharge, soil moisture and terrain (modelled
            inputs)
          </p>
        </div>
              ...
              <div className="min-w-0 pt-1">
                ...
                <div className="mt-4 flex items-baseline justify-between gap-2">
                    ...
                    <span className="text-2xl sm:text-3xl font-extrabold font-mono text-slate-900 tracking-tight shrink-0 tabular-nums">
                      {metric.unavailable ? (
                        ...
                        <AnimatedNumber
                          ...
                          decimals={
                            metric.unit === "m" || metric.unit === "mm" ? 1 : 0
                          }
                          duration={500}
                        />
                      )}
                  ...
                  <div className="flex items-center gap-1 text-xs font-semibold text-slate-600 shrink-0">
                    {metric.unavailable ? null : metric.trend ===
                      "Increasing" ? (

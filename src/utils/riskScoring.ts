...
export const WEIGHTS_WITHOUT_RIVER = { rain: 44, antecedent: 25, soil: 31, river: 0 };
...
export function calculateTransparentRiskScore(inputs: RiskCalculationInputs): TransparentRiskScore {
  ...
  const riverAvailable = riverDischargeRatio !== null && Number.isFinite(riverDischargeRatio);
  ...
  // -------------------------------------------------------------
  const level24 = rain24hMm >= 204.5 ? 4 : rain24hMm >= 115.6 ? 3 : rain24hMm >= 64.5 ? 2 : rain24hMm >= 15.6 ? 1 : 0;
  const level1h = rain1hMm >= 50 ? 4 : rain1hMm >= 30 ? 3 : rain1hMm >= 15 ? 2 : rain1hMm >= 5 ? 1 : 0;
  const level3h = rain3hMm >= 90 ? 4 : rain3hMm >= 60 ? 3 : rain3hMm >= 35 ? 2 : rain3hMm >= 12 ? 1 : 0;
  ...
  else if (rainLevel === 3) rainFactorRatio = 0.75 + frac(rain24hMm, 115.6, 204.5) * 0.24;
  else if (rainLevel === 2) rainFactorRatio = 0.5 + frac(rain24hMm, 64.5, 115.6) * 0.24;
  else if (rainLevel === 1) rainFactorRatio = 0.2 + frac(rain24hMm, 15.6, 64.5) * 0.29;
  else rainFactorRatio = Math.min(0.18, (rain24hMm / 15.6) * 0.18);

  const imdNames = ['Light or nil', 'Moderate', 'Heavy', 'Very Heavy', 'Extremely Heavy'];
  const imdRanges = ['< 15.6 mm', '15.6 - 64.4 mm', '64.5 - 115.5 mm', '115.6 - 204.4 mm', '>= 204.5 mm'];
  ...
  if (rain72hAntecedentMm >= 160) {
    antRatio = 1.0;
    antThresholdText = 'Heuristic: critical antecedent load (>= 160 mm in 72 h)';
    antWatchExceeded = true;
  } else if (rain72hAntecedentMm >= 80) {
    antRatio = 0.65 + ((rain72hAntecedentMm - 80) / 80) * 0.34;
    antThresholdText = 'Heuristic: elevated antecedent load (80 - 160 mm in 72 h)';
    antWatchExceeded = true;
  } else if (rain72hAntecedentMm >= 35) {
    antRatio = 0.3 + ((rain72hAntecedentMm - 35) / 45) * 0.34;
    antThresholdText = 'Heuristic: moderate antecedent moisture (35 - 80 mm in 72 h)';
  } else {
  ...
  if (soilMoistureSaturationPct >= 85) {
    ...
    soilThresholdText = 'Heuristic: near saturation (>= 85% of assumed field capacity)';
    soilWatchExceeded = true;
  } else if (soilMoistureSaturationPct >= 70) {
    soilRatio = 0.6 + ((soilMoistureSaturationPct - 70) / 15) * 0.24;
    soilThresholdText = 'Heuristic: high saturation (70 - 85% of assumed field capacity)';
    soilWatchExceeded = true;
  } else if (soilMoistureSaturationPct >= 50) {
    soilRatio = 0.3 + ((soilMoistureSaturationPct - 50) / 20) * 0.29;
    soilThresholdText = 'Heuristic: moderate (50 - 70% of assumed field capacity)';
  } else {
    soilRatio = Math.min(0.28, (soilMoistureSaturationPct / 50) * 0.28);
    soilThresholdText = 'Heuristic: infiltration buffer available (< 50% of assumed field capacity)';
  }
  ...
  if (riverAvailable) {
    ...
    if (r >= 2.8) {
      ...
      riverThresholdText = 'Heuristic: moderate rise (1.3x - 1.8x recent median)';
    } else {
      riverRatio = Math.min(0.28, Math.max(0, (r / 1.3) * 0.28));
      riverThresholdText = 'Heuristic: nominal flow (< 1.3x recent median)';
    }
  } else {
    riverThresholdText = 'Excluded: river discharge unavailable, weights renormalized';
  }
  const riverPoints = riverAvailable ? Math.round(riverRatio * W.river) : 0;

  const totalScore = Math.min(100, Math.max(0, rainPoints + antPoints + soilPoints + riverPoints));
  ...
  const signals = [rainWatchExceeded, antWatchExceeded, soilWatchExceeded, ...(riverAvailable ? [riverWatchExceeded] : [])];
  ...
  const factors: RiskFactorItem[] = [
    ...
    },
    {
      ...
      description: 'Three-day rainfall total as a proxy for how wet the catchment already is. Cut-offs are heuristic and uncalibrated.',
    },
    {
      id: 'soil-moisture',
    ...
    },
    {
      ...
      rawValue: riverAvailable ? `${(riverDischargeRatio as number).toFixed(2)}x recent median` : 'Unavailable',
